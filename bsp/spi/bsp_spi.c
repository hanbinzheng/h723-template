...
{
	assert(config != NULL && config->hspi != NULL && idx < SPI_INST_MAX_NUM);

	/* check repetition */
	for (uint8_t i = 0; i < idx; i++) {
		if (spi_inst[i].hspi == config->hspi) {
...
HAL_StatusTypeDef spi_transmit(struct spi_inst *inst, uint8_t *buff, uint16_t len)
{
	...
	if (inst->cs_port == NULL || inst->cs_pin == 0) { /* GPIO_PIN_0 is (uint16_t)1 */
		return HAL_SPI_Transmit(inst->hspi, buff, len, SPI_TIMEOUT_MS);
	}

	HAL_StatusTypeDef ret = HAL_ERROR;
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_RESET);
	ret = HAL_SPI_Transmit(inst->hspi, buff, len, SPI_TIMEOUT_MS);
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_SET);
	return ret;

	/* reserved modes: need manually select CS but not manually deselect CS */
	/* HAL_SPI_TransmitReceive_IT(inst->hspi, tx_buff, rx_buff, len); */
...
HAL_StatusTypeDef spi_receive(struct spi_inst *inst, uint8_t *buff, uint16_t len)
{
	...
	/* check stm32h7xx_hal_gpio.h/c for GPIO_TypeDef and HAL_GPIO_WritePin() */
	if (inst->cs_port == NULL || inst->cs_pin == 0) { /* GPIO_PIN_0 is (uint16_t)1 */
		return HAL_SPI_Receive(inst->hspi, buff, len, SPI_TIMEOUT_MS);
	}

	HAL_StatusTypeDef ret = HAL_ERROR;
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_RESET);
	ret = HAL_SPI_Receive(inst->hspi, buff, len, SPI_TIMEOUT_MS);
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_SET);
	return ret;
...
HAL_StatusTypeDef spi_transceive(struct spi_inst *inst, uint8_t *tx_buff, uint8_t *rx_buff,
				 uint16_t len)
{
	assert(inst != NULL && tx_buff != NULL && rx_buff != NULL && inst->hspi != NULL);

	/* check stm32h7xx_hal_gpio.h/c for GPIO_TypeDef and HAL_GPIO_WritePin() */
	if (inst->cs_port == NULL || inst->cs_pin == 0) { /* GPIO_PIN_0 is (uint16_t)1 */
		return HAL_SPI_TransmitReceive(inst->hspi, tx_buff, rx_buff, len, SPI_TIMEOUT_MS);
	}

	HAL_StatusTypeDef ret = HAL_ERROR;
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_RESET);
	ret = HAL_SPI_TransmitReceive(inst->hspi, tx_buff, rx_buff, len, SPI_TIMEOUT_MS);
	HAL_GPIO_WritePin(inst->cs_port, inst->cs_pin, GPIO_PIN_SET);
	return ret;
